...
trait GenuineEmptinessCheck
{
    private function isGenuinelyEmpty(Node $construct, string $source): bool
    {
        ...
        if ($offset === null) {
            // Невідомий/недійсний рядок (напр. провайдер помилково видав
            // line=0) - НЕ можемо просканувати сирий текст, а отже не
            // можемо ні підтвердити, ні спростувати наявність пояснювального
            // коментаря. Безпечний фолбек - false (= "не вважай genuinely
            // empty", правило просто не спрацює), а не true: інакше баг
            // провайдера з line-номером мовчки перетворюється на false
            // positive, що ще й ігнорує коментар, замість просто "рідше
            // ловити" (false negative - значно безпечніший клас помилки
            // для лінтера).
            return false;
        }
